...
- Windows installer smoke tests now run only in Windows Sandbox or explicitly ephemeral CI workers,
  contain product processes in a strict kill-on-close Job Object, and restrict cleanup to owned
  paths while allowing NSIS to manage its installer process topology inside the outer sandbox.
