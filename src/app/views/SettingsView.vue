...
const windowsUpdateTrigger = ref<HTMLButtonElement>()
...
const settingsSections = computed<SettingsSectionLink[]>(() => [
  ...
  { id: 'settings-updates', label: '应用更新', hint: '签名版本' },
...
async function checkWindowsUpdate() {
  ...
  try {
    ...
    await nextTick()
    windowsUpdateTrigger.value?.focus()
  }
}
...
function formatUpdatePublication(value: string | null) {
  ...
  return Number.isFinite(timestamp) ? formatBackupTime(timestamp) : ''
}
...
<template>
  <main class="settings-page">
    ...
    >
      ...
      <div
        v-if="windowsUpdateStatus"
        class="update-contract"
      >
        <span class="update-contract-icon"><Laptop :size="21" /></span>
        <div>
          <strong>
            {{ windowsUpdateStatus.enabled ? '已接入签名更新通道' : '当前安装包未接入自动更新' }}
          </strong>
          <p>当前版本 {{ windowsUpdateStatus.currentVersion }}</p>
          <small v-if="windowsUpdateStatus.enabled">
        ...
        <div
          v-if="windowsUpdateStatus.enabled"
          class="update-actions"
        >
          <button
            ref="windowsUpdateTrigger"
            type="button"
            :disabled="checkingWindowsUpdate || installingWindowsUpdate"
            @click="checkWindowsUpdate"
          >
            <RotateCcw :size="16" />
            {{ checkingWindowsUpdate ? '正在检查…' : '检查更新' }}
          </button>
          <button
            v-if="windowsUpdateReport?.available && windowsUpdateReport.version"
            type="button"
            class="primary"
            :disabled="checkingWindowsUpdate || installingWindowsUpdate"
            @click="installWindowsUpdate"
          >
            <ArchiveRestore :size="16" />
            {{ installingWindowsUpdate ? '正在下载并验证…' : `安装 ${windowsUpdateReport.version}` }}
          </button>
        </div>
      </div>
      <p
        v-if="windowsUpdateMessage"
      ...
      >
        ...
        <small v-if="windowsUpdateReport?.publishedAt">
          发布时间 {{ formatUpdatePublication(windowsUpdateReport.publishedAt) }}
        </small>
      </p>
    </section>
...
.update-panel { margin-top: 16px; padding: 26px; }
.update-panel > header { align-items: center; margin-bottom: 16px; }
.update-panel > header p { margin: 0 0 8px; color: var(--cinnabar); font-size: 12px; font-weight: 800; letter-spacing: .14em; text-transform: uppercase; }
.update-panel > header span { max-width: 690px; font-size: 12px; }
.update-panel > header > svg { flex: 0 0 auto; color: var(--green-deep); }
...
@media (max-width: 760px) { .update-contract { grid-template-columns: 42px 1fr; }.update-actions { grid-column: 1 / -1; flex-direction: column; }.update-actions button { width: 100%; } }
