...
const {
  ...
  initialQuery: filters.value,
  pageSize: 20,
  listPage: async input => previewMode.value
    ? { ok: true as const, data: previewPage() }
    : normalizeAppResult(await commands.reviewHistoryList(input)),
})
...
onMounted(() => { void replaceHistory(filters.value) })
</script>
