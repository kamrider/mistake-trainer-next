...
const {
  ...
  loadCandidates,
  changeSource: loadCandidateSource,
  replacePreview: replacePreviewCandidates,
  toggle: toggleCandidate,
  select: selectCandidates,
  clear: clearCandidateSelection,
} = useExportCandidateSelection({
  load: async source => normalizeAppResult(await commands.exportCandidates(source)),
})
const {
  current: deleteConfirmation,
  ask: askDeleteConfirmation,
  confirm: confirmSnapshotDelete,
  cancel: cancelSnapshotDelete,
} = useActionConfirmation()
const snapshotMutations = useExportSnapshotMutations({
  snapshots: () => snapshots.value,
  deletedSnapshots: () => deletedSnapshots.value,
  onSnapshotsChange: items => { snapshots.value = items },
  onDeletedSnapshotsChange: items => { deletedSnapshots.value = items },
  onTrashLoadedChange: loaded => { trashLoaded.value = loaded },
  onError: message => { errorMessage.value = message },
  onStatus: message => { generatedMessage.value = message },
  scheduleSync: () => syncController?.scheduleMutation(),
  blocked: () => loading.value,
  confirmDelete: snapshot => askDeleteConfirmation({
    eyebrow: '导出快照 · 删除确认',
    title: `将“${snapshot.title}”移入回收区？`,
    description: '原题不会受到影响；快照会进入回收区，之后仍可恢复。',
    confirmLabel: '移入回收区',
    cancelLabel: '保留快照',
    tone: 'danger',
  }),
  createOperation: async input => normalizeAppResult(await commands.exportCreate(input)),
  generateOperation: async snapshotId => normalizeAppResult(await commands.exportGenerate(snapshotId)),
  deleteOperation: async snapshotId => normalizeAppResult(await commands.exportDelete(snapshotId)),
  restoreOperation: async snapshotId => normalizeAppResult(await commands.exportRestore(snapshotId)),
  listTrash: async () => normalizeAppResult(await commands.exportTrashList()),
})
const {
  saving,
  generatingId,
  ...
  restoringId,
  operationBusy,
  createSnapshot: persistSnapshot,
  generateSnapshot,
  deleteSnapshot,
  restoreSnapshot,
} = snapshotMutations
const durableTransitionBlockedMessage = '导出操作正在进行，请等待完成后再离开。'
const { attemptLeave: attemptDurableTransition } = useDurableActionGuard({
  busy: () => operationBusy.value,
  onBlocked: () => {
...
async function load() {
  if (operationBusy.value || candidateLoading.value || reportRefreshInFlight) return
...
function changeCandidateSource(source: ExportCandidateSource) {
  ...
  if (isDevelopmentPreview()) {
    replacePreviewCandidates(source, developmentCandidates(source))
    return
  }
  void loadCandidateSource(source)
}
  ...
  <main class="report-page">
    <header class="page-heading">
      ...
      <div class="heading-actions">
        ...
        <button
          type="button"
          :disabled="loading || candidateLoading || operationBusy"
          @click="load"
        >
          <RotateCcw
            :size="16"
    ...
    <nav
      class="report-section-nav"
      aria-label="报告页面目录"
    >
      <a href="#report-overview">学习概览</a>
      <a href="#export-center">导出中心</a>
    </nav>
    ...
    <section
      id="export-center"
      class="paper-panel export-panel"
      aria-labelledby="export-center-title"
    >
      <header>
        <div>
          <p>导出中心</p><h2 id="export-center-title">
            保存配置并生成学习材料
          </h2><span>先保存可同步快照，再由 Rust 解密并原子生成 DOCX 或原图文件夹。</span>
        </div><FileArchive
          :size="24"
          aria-hidden="true"
        />
      ...
      <div class="export-form">
        ...
        <div class="configuration-step">
          ...
          <button
            type="button"
            :disabled="loading || operationBusy || candidateLoading || orderedSelectedProblemIds.length === 0 || !title.trim()"
      ...
      <ExportSnapshotHistory
        ...
        :operation-busy="loading || operationBusy"
...
.report-section-nav { display: flex; gap: 8px; width: fit-content; margin: -12px 0 24px; padding: 5px; border: 1px solid var(--line); border-radius: 999px; background: rgba(255,253,247,.72); }
.report-section-nav a { display: inline-flex; min-height: 44px; align-items: center; padding: 0 15px; color: var(--green-deep); border-radius: 999px; font-size: 13px; font-weight: 750; text-decoration: none; }
.report-section-nav a:hover,.report-section-nav a:focus-visible { background: var(--green-soft); }
...
.bar-column small, .bar-value { color: var(--ink-muted); font-size: 12px; } .subject-list { display: grid; gap: 19px; margin: 24px 0 0; padding: 0; list-style: none; }
...
.subject-list b { display: block; height: 100%; border-radius: inherit; background: var(--cinnabar); } .subject-list small { color: var(--ink-muted); font-size: 12px; }
...
.step-heading small { margin-top: 3px; color: var(--ink-muted); font-size: 12px; }
.candidate-error { display: flex; gap: 12px; align-items: center; justify-content: space-between; margin: 0 0 12px; padding: 10px 12px; color: #843d2c; border: 1px solid rgba(185, 88, 63, .24); border-radius: 10px; background: rgba(185, 88, 63, .08); font-size: 12px; }
.candidate-error button { min-height: 44px; padding: 0 10px; color: #843d2c; background: transparent; white-space: nowrap; }
...
.export-form label,
.export-form legend { color: var(--ink-muted); font-size: 13px; font-weight: 700; }
...
.layout-option strong { color: var(--ink); font-size: 13px; }
.layout-option small { margin-top: 4px; color: var(--ink-muted); font-size: 12px; font-weight: 500; line-height: 1.4; }
.configuration-step > button { min-height: 44px; padding: 0 15px; color: white; border-color: var(--green-deep); background: var(--green-deep); white-space: nowrap; transition: transform var(--motion-feedback) var(--ease-standard), box-shadow var(--motion-standard) var(--ease-standard); }
