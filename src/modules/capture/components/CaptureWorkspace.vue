...
import {
  ...
  Images, LayoutGrid, ListPlus, LockKeyhole, MoreHorizontal, Plus, QrCode, Save, Smartphone,
  Sparkles, Trash2, Undo2, UploadCloud, X,
} from '@lucide/vue'
...
const layoutConfirmOpen = ref(false)
const batchMenuId = ref('')
...
const incompleteDraftSummaries = computed(() => props.detail?.drafts
  ...
  .filter(Boolean) ?? [])
const affectedNoteCount = computed(() => props.detail?.drafts
  .filter(draft => draft.note.trim() || draft.tags.length)
  .length ?? 0)
...
watch(() => props.detail, (detail) => {
  ...
  pendingBatchSubject.value = detail.batch.subject
...
function requestLayout() {
  if (props.detail?.drafts.length) {
    layoutConfirmOpen.value = true
    return
  }
...
function requestDiscard(batch: CaptureBatchSummary) {
  batchMenuId.value = ''
  if (window.confirm(`删除“${batchTitle(batch)}”？只会清理未被其他草稿或题库引用的图片。`)) {
        ...
        >
          ...
          >
            ...
            <button
              ...
              @click.stop="batchMenuId = batchMenuId === batch.id ? '' : batch.id"
            >
      ...
      <template v-else-if="detail.batch.state === 'organizing'">
          ...
          <div class="batch-subject-controls">
            <div class="batch-subject-options">
              <button
                ...
                :disabled="busy"
                @click="pendingBatchSubject = subject"
              >
                {{ subject }}
              </button>
            </div>
        ...
        >
          <div>
            ...
            <p>后续才会加入 OCR、内容理解、题答匹配和一键导出。当前版本不会运行这条流程，也不会要求下载 small / medium 模型。</p>
          </div>
        ...
        >
          ...
          <button
            ...
            @click="requestLayout"
          >
            <LayoutGrid :size="16" />{{ detail.drafts.length ? '重新分组全部图片' : '按模板生成题卡' }}
          </button>
        </section>

        <section
          v-if="layoutConfirmOpen"
          class="layout-impact"
          role="alertdialog"
          aria-modal="true"
        ...
        >
          ...
          <p>{{ detail.drafts.length }} 张题卡会被重新生成；{{ affectedNoteCount }} 张含标签或笔记的题卡会清空这些逐题信息。</p>
          <strong>{{ detail.items.length }} 张图片都会保留，不会删除原图或切图。</strong>
          <div>
            <button
              type="button"
              :disabled="busy"
        ...
        >
          ...
          >
            <div class="strip-heading">
              <div><p>素材牌库</p><span>{{ pointerDrag.drag.hoveredTarget?.kind === 'unassigned' ? '松开即可把图片移回素材区' : unassignedItems.length ? '先选中图片，再明确设置用途或加入题卡' : '拖回的图片会重新出现在这里' }}</span></div><strong>{{ unassignedItems.length }}</strong>
            </div>
            <TransitionGroup
...
.capture-toolbar { margin-top:31px; padding:15px; border:1px solid var(--line); border-radius:15px; background:rgba(255,253,247,.58); }.capture-toolbar-heading { display:flex; justify-content:space-between; gap:18px; align-items:center; margin-bottom:10px; }.capture-toolbar-heading p,.capture-toolbar-heading span { margin:0; }.capture-toolbar-heading p { font-weight:800; }.capture-toolbar-heading span { color:var(--ink-muted); font-size:10px; }.capture-toolbar-actions { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)) minmax(180px,.75fr); gap:10px; }.capture-toolbar button,.tool-hint { display:flex; gap:11px; align-items:center; min-height:54px; padding:0 17px; color:var(--ink); border:1px solid var(--line); border-radius:13px; background:rgba(255,253,247,.8); text-align:left; }.capture-toolbar button { justify-content:flex-start; cursor:pointer; }.capture-toolbar .primary-tool { color:var(--paper); border-color:var(--green-deep); background:var(--green-deep); }.capture-toolbar button span,.tool-hint span { display:grid; gap:2px; }.capture-toolbar strong,.tool-hint strong { font-size:13px; }.capture-toolbar small,.tool-hint small { opacity:.68; font-size:10px; }
...
.layout-impact { position:fixed; z-index:95; top:50%; left:50%; width:min(520px,calc(100vw - 40px)); padding:24px; box-sizing:border-box; border:1px solid rgba(33,51,45,.18); border-radius:18px; background:var(--paper); box-shadow:0 0 0 9999px rgba(25,35,31,.46),0 24px 70px rgba(20,31,27,.28); transform:translate(-50%,-50%); }.layout-impact h2,.layout-impact p,.layout-impact strong { display:block; margin:0; }.layout-impact h2 { font-size:23px; }.layout-impact p { margin-top:12px; color:var(--ink-muted); font-size:12px; line-height:1.6; }.layout-impact strong { margin-top:9px; color:var(--green-deep); font-size:12px; }.layout-impact>div { display:flex; gap:9px; justify-content:flex-end; margin-top:20px; }.layout-impact button { min-height:40px; padding:0 15px; border:1px solid var(--line); border-radius:999px; background:var(--paper); font-weight:760; cursor:pointer; }.layout-impact button:first-child { color:var(--paper); border-color:var(--green-deep); background:var(--green-deep); }
.recognition-result { display: flex; gap: 18px; align-items: center; justify-content: space-between; margin-top: 16px; padding: 15px 18px; border: 1px solid rgba(79,128,110,.28); border-radius: 14px; background: rgba(225,235,229,.72); }.recognition-result>div { display: flex; gap: 11px; align-items: flex-start; color: var(--green-deep); }.recognition-result p,.recognition-result strong,.recognition-result span,.recognition-result small { display: block; margin: 0; }.recognition-result span { margin-top: 2px; color: var(--ink); font-size: 12px; }.recognition-result small { margin-top: 3px; color: var(--ink-muted); }.recognition-result button { display: inline-flex; flex: 0 0 auto; gap: 7px; align-items: center; min-height: 38px; padding: 0 14px; border: 1px solid rgba(33,51,45,.24); border-radius: 999px; background: var(--paper); color: var(--green-deep); font-weight: 740; cursor: pointer; }
.future-recognition-note { display:flex; gap:14px; align-items:center; justify-content:space-between; margin-top:8px; padding:9px 13px; color:var(--ink-muted); border-left:3px solid var(--sand); background:rgba(244,241,232,.42); }.future-recognition-note h2,.future-recognition-note p { margin:0; }.future-recognition-note .eyebrow { color:var(--ink-muted); font-size:9px; font-weight:850; letter-spacing:.08em; }.future-recognition-note h2 { display:none; }.future-recognition-note div>p:last-child { max-width:850px; margin-top:2px; font-size:10px; line-height:1.45; }.future-mode-badge { flex:0 0 auto; padding:5px 9px; color:var(--ink-muted); border:1px solid rgba(33,51,45,.17); border-radius:999px; background:rgba(255,253,247,.68); font-size:9px; font-weight:820; }
.batch-subject-bar { display:grid; grid-template-columns:minmax(220px,.6fr) minmax(0,1.4fr); gap:18px; align-items:center; margin-top:24px; padding:17px 19px; border:1px solid rgba(33,51,45,.16); border-radius:5px 17px 17px; background:linear-gradient(120deg,rgba(225,235,229,.7),rgba(255,253,247,.74)); box-shadow:var(--shadow-soft); }.batch-subject-bar p,.batch-subject-bar strong,.batch-subject-bar span { display:block; margin:0; }.batch-subject-bar p { color:var(--cinnabar); font-size:10px; font-weight:840; letter-spacing:.12em; }.batch-subject-bar strong { margin-top:3px; font-family:var(--font-serif); font-size:21px; }.batch-subject-bar span { margin-top:3px; color:var(--ink-muted); font-size:10px; }.batch-subject-controls { display:grid; gap:8px; }.batch-subject-options { display:flex; gap:7px; flex-wrap:wrap; justify-content:flex-end; }.batch-subject-options button { min-width:52px; min-height:34px; padding:0 12px; color:var(--ink-muted); border:1px solid var(--line); border-radius:999px; background:var(--paper); cursor:pointer; transition:transform var(--motion-feedback),color var(--motion-feedback),background var(--motion-feedback); }.batch-subject-options button:hover { transform:translateY(-1px); }.batch-subject-options button.selected { color:var(--paper); border-color:var(--green-deep); background:var(--green-deep); }.batch-subject-confirm { display:flex; justify-content:flex-end; gap:10px; align-items:center; min-height:34px; }.batch-subject-confirm button { min-height:34px; padding:0 13px; color:var(--paper); border:0; border-radius:999px; background:var(--green-deep); font-weight:760; cursor:pointer; }
...
.material-actions p strong { overflow:hidden; font-size:11px; text-overflow:ellipsis; white-space:nowrap; }
.material-actions p span { margin-top:2px; color:var(--ink-muted); font-size:9px; }
.material-role-actions,.material-draft-actions { display:flex; gap:6px; flex-wrap:wrap; }
.material-actions button { min-height:32px; padding:0 10px; color:var(--green-deep); border:1px solid rgba(33,51,45,.18); border-radius:999px; background:var(--paper); font-size:10px; font-weight:740; cursor:pointer; }
...
.commit-dock ul { grid-column:1/-1; display:flex; gap:6px; flex-wrap:wrap; margin:4px 0 0; padding:0; list-style:none; }.commit-dock li { padding:4px 7px; color:#7b493b; border-radius:999px; background:rgba(246,226,216,.72); font-size:9px; }
...
@media (max-width: 720px) { .capture-next { padding: 30px 20px 170px; }.inbox-hero,.workbench-header,.new-batch-card { align-items: stretch; flex-direction: column; }.new-batch-card form { grid-template-columns: 1fr; flex-direction: column; }.new-batch-card select { width: 100%; }.batch-grid { grid-template-columns: 1fr; }.capture-toolbar-actions { grid-template-columns:1fr; }.capture-toolbar-heading { align-items:flex-start; flex-direction:column; }.workbench-stats { align-self: stretch; }.workbench-stats span { flex: 1; }.layout-bar select { min-width: 100%; }.commit-dock { align-items: stretch; flex-direction: column; }.commit-dock button { width: 100%; }.lan-dialog { padding: 28px 20px; }.lan-dialog h2 { font-size: 28px; }.lan-actions>* { width: 100%; } }
